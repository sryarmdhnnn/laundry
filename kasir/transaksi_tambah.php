...
$outlet = ambilsatubaris($conn, 'SELECT nama_outlet from outlet WHERE id_outlet = ' . $id_outlet);
$member = ambilsatubaris($conn, 'SELECT nama_member from member WHERE id_member = ' . $id_member);
$paket = ambildata($conn, 'SELECT * FROM paket WHERE id_outlet = ' . $id_outlet);
if (isset($_POST['btn-simpan'])) {
    ...
    $query = "INSERT INTO transaksi (id_outlet,kode_invoice,id_member,tgl,batas_waktu,biaya_tambahan,diskon,pajak,status,status_bayar,id_user) VALUES ('$id_outlet','$kode_invoice','$id_member','$tgl_sekarang','$batas_waktu','$biaya_tambahan','$diskon','$pajak','baru','belum','$id_user')";

    $execute = bisa($conn, $query);
    if ($execute == 1) {
        $id_paket = $_POST['id_paket'];
        $qty = $_POST['qty'];
        $hargapaket = ambilsatubaris($conn, 'SELECT harga from paket WHERE id_paket = ' . $id_paket);
        ...
        $sqlDetail = "INSERT INTO detail_transaksi (id_transaksi,id_paket,qty,total_harga) VALUES ('$id_transaksi','$id_paket','$qty','$total_harga')";
