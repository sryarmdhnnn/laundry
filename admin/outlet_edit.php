...
$query = 'SELECT outlet.*, user.nama_user,user.id_user FROM outlet LEFT JOIN user ON user.id_outlet = outlet.id_outlet WHERE id_outlet = ' . stripslashes($_GET['id']);
$data = ambilsatubaris($conn, $query);

$query2 = 'SELECT user.*,outlet.nama_outlet FROM outlet RIGHT JOIN user ON user.id_outlet = outlet.id_outlet WHERE user.role = "owner" order by user.id_outlet asc';
...
if (isset($_POST['btn-simpan'])) {
    ...
    if ($_POST['id_owner_new']) {
        $query2 = "UPDATE user SET id_outlet = '" . stripslashes($_GET['id']) . "' WHERE id_user = " . $_POST['id_owner_new'];
        $query3 = "UPDATE user SET id_outlet = NULL WHERE id_user = " . stripslashes($data['id_user']);
        $execute3 = bisa($conn, $query3);
    } else {
        $query2 = "UPDATE user SET id_outlet = '" . stripslashes($_GET['id']) . "' WHERE id_user = " . stripslashes($_POST['id_owner']);
    }
                ...
                <form method="post" action="">
                    ...
                    <?php else : ?>
                        <div class="form-group">
                            ...
                            <select name="id_owner_new" class="form-control">
